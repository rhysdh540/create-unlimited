...
public class TrackPlacementMixin {
    @Shadow(remap = false) public static PlacementInfo cached;
    @Shadow(remap = false) static BlockPos hoveringPos;
    @Shadow(remap = false) static boolean hoveringMaxed;
    ...
    public static PlacementInfo tryConnect(Level level, Player player, BlockPos pos2, BlockState state2, ItemStack stack, boolean girder, boolean maximiseTurn) {
        ...
        CUConfig.PlacementCheck placementCheck = CUConfig.placementChecksEnabled.get();
        boolean check = placementCheck == CUConfig.PlacementCheck.ON || (placementCheck == CUConfig.PlacementCheck.SURVIVAL_ONLY && !player.isCreative());
        ...
        if (slope) {
            if(check) {
                ...
                if (Mth.equal(normal1.dot(normal2), 0)) return info.withMessage("opposing_slopes");
            ...
            intersect = CreateUnlimited.intersect(end1, end2, normedAxis1, normedAxis2, plane);
            ...
            if ((intersect[0] < 0 || intersect[1] < 0) && check)
                return info.withMessage("too_sharp")
                        .tooJumbly();
            if (turnSize < 2 && check)
                return info.withMessage("too_sharp");
        ...
        if (!parallel) {
            ...
            if ((absAngle < 60 || absAngle > 300) && check)
                return info.withMessage("turn_90")
                        .tooJumbly();

            intersect = CreateUnlimited.intersect(end1, end2, normedAxis1, normedAxis2, Direction.Axis.Y);
            ...
            if (turnSize < minTurnSize && check)
                return info.withMessage("too_sharp");
            if (turnSize < turnSizeToFitAscend && check)
                return info.withMessage("too_steep");
