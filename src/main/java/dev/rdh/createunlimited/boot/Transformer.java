...
public class Transformer implements IMixinService, IClassBytecodeProvider, IExtension, IMixinConfigPlugin {
	private static final ILogger LOGGER = MixinService.getService().getLogger("TestMod/Boot");
	private static final ClassLoader CL = Transformer.class.getClassLoader();

	// region IMixinService and IClassBytecodeProvider
	...
	public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
		LOGGER.info("Received mixin {} -> {}", mixinClassName, targetClassName);
		boolean result = mixinClassName.endsWith("$Patched") || !patches.containsKey(mixinClassName);
		LOGGER.info("  {}", result ? "Applying" : "Not applying");
		return result;
	}
	...
	private static String getPlatform() {
		if (classExists("net.minecraftforge.fml.loading.FMLLoader")) {
			return "forge";
		} else if (classExists("net.neoforged.fml.loading.FMLLoader")) {
			return "neoforge";
		} else if (classExists("net.fabricmc.loader.FabricLoader")) {
			return "fabric";
		}
	...
	// region Patch Loader
	private static final Map<String, byte[]> patches = getPatches();

	private static Map<String, byte[]> getPatches() {
	 ...
	 */
	private Set<String> getTransformedClasses() {
		try {
	...
	private List<String> getTransformedMixins(String packag) {
		try {
			Map<String, ClassNode> mixins = new HashMap<>();
			for(String className : patches.keySet()) {
				byte[] classBytes = CL.getResourceAsStream(className.replace('.', '/') + ".class")
						.readAllBytes();
				ClassNode classNode = new ClassNode();
				...
				if(getMixin(classNode) == null) continue;
				...
				mixins.put(classNode.name.replace('/', '.'), classNode);
			}


			for(ClassNode classNode : mixins.values()) {
				if (!addClass(classNode)) {
					LOGGER.warn("Failed to add mixin class {} to Transformer", classNode.name);
				}
			}

			List<String> mixinsNoPackage = new ArrayList<>();
			for(String className : mixins.keySet()) {
				if (!className.startsWith(packag)) {
					// should never happen since we filtered them out above
					throw new AssertionError(className);
				}

				mixinsNoPackage.add(className.substring(packag.length() + 1));
			}

			return mixinsNoPackage;
		} catch (Exception e) {
			throw unchecked(e);
		}
	}

	private static AnnotationNode getMixin(ClassNode classNode) {
